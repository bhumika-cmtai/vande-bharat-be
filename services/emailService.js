...
const generateEmailHTML = (subject, message, imageUrl) => {
  const appName = process.env.APP_NAME || 'VandeBharat';
...
export const sendBulkEmail = async (emailList, subject, message, imageUrl) => {
  ...
  try {
    ...
    const mailOptions = {
      from: `"${process.env.APP_NAME || 'Vande Bharat'}" <${process.env.MAIL_USER}>`,
...
export const sendOrderConfirmationEmail = async (userEmail, order) => {
  ...
  try {
    ...
    const subject = `Your ${process.env.APP_NAME || 'Vande Bharat'} Order Confirmation #${order._id}`;
